...
> **Hata alırsanız:** PowerShell'i kapatıp yeniden açın. Hâlâ hata alırsanız ilgili aracın kurulum klasörünü PATH'e manuel ekleyin (Bkz. Bölüm 4 — Yöntem B).
...
Debug kartınızı (CMSIS-DAP / ABOV debug board) USB ile bilgisayara bağlayın. Kart ile mikrodenetleyici arasındaki bağlantıların doğru olduğundan emin olun:

| Debug Kartı Pini | MCU Pini |
|---|---|
| SWDIO | SWDIO |
| SWCLK | SWCLK |
| GND | GND |
...
### `cmake` komutu tanınmıyor
CMake kurulduktan sonra PowerShell yeniden açılmamış olabilir. Pencereyi kapatıp yeniden açın.

### `arm-none-eabi-gcc` bulunamıyor
Toolchain kurulumunda **"Add to PATH"** seçeneği işaretlenmemiştir. Kurulum klasörünü (örn. `C:\Program Files (x86)\Arm GNU Toolchain\bin`) PATH'e manuel ekleyin.
